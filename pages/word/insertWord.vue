<template>
	<view class="word-info-box">
		...
		<view class="word-example-box">
			<view class="word-example" v-for="(item, index) in wordExampleIndex" :key="index">
		...
		<textarea class="word-note-input" type="text" v-model="wordNote" auto-height />
...
<script>
	export default {
		...
		methods: {
			...
			addWordExample() {
				this.wordExampleIndex.push(this.wordExampleIndex[this.wordExampleIndex.length - 1]);
				this.wordExampleBtn[1] = 1;
			},
			subWordExample() {
				if (this.wordExampleIndex.length == 2) {
					this.wordExampleBtn[1] = 0;
				}

				if (this.wordExampleIndex.length > 1) {
					this.wordExampleIndex.pop(this.wordExampleIndex[this.wordExampleIndex.length - 1]);
					this.wordExampleF.pop(this.wordExampleF[this.wordExampleF.length - 1])
					this.wordExampleC.pop(this.wordExampleC[this.wordExampleC.length - 1])
				}
			},
			submit() {
				...
				if (this.wordName === '') {
					plus.nativeUI.alert('单词存在空值，无法添加');
				...
				plus.sqlite.executeSql({
					...
					fail(e) {
						if (e.message ===
							'android.database.sqlite.SQLiteConstraintException: UNIQUE constraint failed: wordtable.word (code 1555 SQLITE_CONSTRAINT_PRIMARYKEY),https://ask.dcloud.net.cn/article/282'
						) {
							plus.nativeUI.alert('单词已经存在，请勿重复添加');
						} else {
							plus.nativeUI.alert('插入失败: ' + JSON.stringify(e));
						}
						console.log('插入失败' + JSON.stringify(e));
					}
				});
